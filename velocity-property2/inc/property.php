            ...
            <div class="col-3 pe-0">
				...
				if (has_post_thumbnail($post->ID)) {
					...
					echo '<a href="' . get_the_permalink($post->ID) . '">';
						echo '<img src="' . $thumbnail_url . '" class="img-fluid" style="width: 100%; aspect-ratio:1/1; object-fit: cover;" alt="' . get_the_title($post->ID) . '">';
					echo '</a>';
				}
				?>
            </div>
...
function property_loop($atts) {
	...
	$class = $atribut['class'] ? ' ' . $atribut['class'] : '';
	...
	echo '<article class="'.$class.'">';
		echo '<div class="card h-100 border-0 listing-item">';
			if (has_post_thumbnail()) {
				echo '<div class="listing-image ratio ratio-4x3">';
				echo '<a href="' . $link . '" class="text-decoration-none text-dark">';
					echo '<img src="' . get_the_post_thumbnail_url($post_id, 'full') . '" class="img-fluid rounded" alt="' . $title . '">';
				echo '</a>';
				echo '</div>';
			}
			echo '<div class="listing-content p-3">';
				echo '<h4 class="card-title fw-bold mb-2">';
					echo '<a href="' . $link . '" class="text-decoration-none text-dark">' . wp_trim_words($title, 10, '...') . '</a>';
				echo '</h4>';
				echo '<div class="py-2"><i class="fa fa-map-o pe-2" aria-hidden="true"></i>'.$lokasi.'</div>';
				echo '<div class="listing-meta d-flex justify-content-between align-items-center mb-2">';
					echo '<span class="fs-6 pe-2"><i class="fa fa-bed"></i> ' .$kamar_tidur. '</span>';
					echo '<span class="fs-6 pe-2"><i class="fa fa-home"></i> ' .$jumlah_lantai. '</span>';
					echo '<span class="fs-6 pe-2"><i class="fa fa-money-bill"></i> ' . velocity_harga($post_id) . '</span>';
					echo '<span class="'.$button.' pe-2"><a class="btn btn-primary" href="'. $link .'">View</a></span>';
				echo '</div>';
			echo '</div>';
		echo '</div>';
	echo '</article>';
